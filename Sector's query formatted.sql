...
SELECT MAX(asof_dt) as max_asof_dt
into   #temp
...
select CAST(memb_code as varchar(50)) as portfolio_name,
       sum(case
                when portfolio_limit_name = 'SpreadPC1' then portfolio_limit.value
                else 0 end) as mpc1_limit,
       sum(case
                when portfolio_limit_name = 'TotalTE' then portfolio_limit.value
                else 0 end) as te_limit
into   #temp_mpc1_limit
  from portfolio_limit
 where portfolio_limit.p_end_dt = '12/31/9999'
   and portfolio_limit.portfolio_limit_name in ( 'SpreadPC1', 'TotalTE' )
 group by memb_code;

--Pull Sector Holdings
select dn_security_ts.pgim_sector,
       ...
       CASE
            WHEN dn_security.currency IN ( 'USD', 'GBP', 'EUR' ) THEN dn_security.currency
            ELSE 'Other' END as currency_group,
       dn_security.currency,
       CASE
            WHEN #temp_breakout_define.aaa_aa_breakout = 1 then case
                                                                     when mid_value >= 29 THEN 'AAA'
                                                                     WHEN mid_value >= 26 THEN 'AA'
                                                                     ELSE 'Below AA' END
            WHEN #temp_breakout_define.full_rating_breakout = 1 THEN
                case
                     when sec_type = 'SWAP_CDSWAP' THEN 'CDX/CDS'
                     WHEN sec_type = 'SYNTH_SWAPTION' THEN 'CDX/CDS Options'
                     when mid_value >= 26 THEN ' AAA.AA'
                     WHEN mid_value >= 23 THEN 'A'
                     WHEN mid_value >= 20 THEN 'BBB'
                     WHEN mid_value >= 17 THEN 'BB'
                     WHEN mid_value >= 14 THEN 'B'
                     ELSE 'Below B' END
       ...
       case
            when sum(dn_port_bmrk_position.port_pct_adj_nmv_mv) = 0 THEN 0
            ELSE
                sum(dn_port_bmrk_position.port_pct_adj_nmv_mv * dn_analytic.oas)
                / (sum(dn_port_bmrk_position.port_pct_mv) + 0.0000001) END AS port_oas,
       case
            when sum(dn_port_bmrk_position.bmrk_pct_adj_nmv_mv) = 0 THEN 0
            ELSE
                sum(dn_port_bmrk_position.bmrk_pct_adj_nmv_mv * dn_analytic.oas)
                / (sum(dn_port_bmrk_position.bmrk_pct_mv) + 0.0000001) END AS bmrk_oas
  from dn_security_ts
  JOIN dn_security
    ON dn_security.asset_id                    = dn_security_ts.asset_id
   AND dn_security_ts.asof_dt                  = (select max_asof_dt from #temp)
  JOIN #temp_breakout_define
    ON #temp_breakout_define.pgim_sector       = dn_security_ts.pgim_sector
  ...
  JOIN dn_analytic
    ON dn_analytic.asset_id                    = dn_security_ts.asset_id
   and dn_analytic.asof_dt                     = dn_security_ts.asof_dt
   and dn_analytic.pricing_source              = coalesce(dn_port_bmrk_position.port_pricing_source, dn_port_bmrk_position.bmrk_pricing_source)
  LEFT JOIN dn_analytic as underlying_analytic
    ON underlying_analytic.asset_id            = dn_security.underlying_asset_id
   and underlying_analytic.asof_dt             = dn_security_ts.asof_dt
   and underlying_analytic.pricing_source      = coalesce(dn_port_bmrk_position.port_pricing_source, dn_port_bmrk_position.bmrk_pricing_source)
  JOIN dn_portfolio_group_mandate
    on dn_portfolio_group_mandate.portfolio_id = dn_port_bmrk_position.portfolio_id
  join #temp_mpc1_limit
    ON #temp_mpc1_limit.portfolio_name         = dn_portfolio_group_mandate.portfolio_name
 group by dn_security_ts.pgim_sector,
          dn_portfolio_group_mandate.portfolio_name,
          dn_portfolio_group_mandate.pag_mandate,
          dn_port_bmrk_position.synthetic_look_through,
          CASE
